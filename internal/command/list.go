	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			filterCriteria := v1.ListOptions{
				LabelSelector: fmt.Sprintf("%s=%s", KameletTypeLabel, "sink"),
			}
...
func printKameletList(kameletList *camelkv1.KameletList, options hprinters.PrintOptions) ([]metav1beta1.TableRow, error) {
...
func printKamelet(kamelet *camelkv1.Kamelet, options hprinters.PrintOptions) ([]metav1beta1.TableRow, error) {
	name := kamelet.Name
...
func conditionsValue(conditions []camelkv1.KameletCondition) string {
	var ok int
...
func readyCondition(conditions []camelkv1.KameletCondition) string {
	for _, condition := range conditions {
		if condition.Type == camelkv1.KameletConditionReady {
...
func nonReadyConditionReason(conditions []camelkv1.KameletCondition) string {
	for _, condition := range conditions {
		if condition.Type == camelkv1.KameletConditionReady {
